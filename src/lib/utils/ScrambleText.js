...
/* eslint-disable */
import { getText, emojiSafeSplit } from "./";
...
export var ScrambleTextPlugin = {
  ...
  init: function init(target, value, tween, index, targets) {
    ...
    data.length = splitByChars
      ? data.original.length
      : data.original.join(delim).length;
    data.lengthDif =
      (splitByChars ? data.text.length : data.text.join(delim).length) -
      data.length;
    data.fillChar =
      value.fillChar || (value.chars && ~value.chars.indexOf(" "))
        ? "&nbsp;"
        : "";
  ...
  render: function render(ratio, data) {
    ...
    position =
      length +
      (tweenLength ? (tween._from ? r * r * r : 1 - r * r * r) : 1) * lengthDif;
        ...
        if (charsHaveEmoji) {
          startText = emojiSafeSplit(endText)
            .slice(
              0,
              (position -
                (textHasEmoji ? emojiSafeSplit(str) : str).length +
                0.5) |
                0,
            )
            .join("");
