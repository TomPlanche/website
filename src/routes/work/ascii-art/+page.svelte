<script lang="ts">
  ...
  import {
    ...
    Text,
    ButtonGrid,
    type ButtonGridClickEvent, IntervalSlider, type ListOptions, List
  ...
  const sketch = (p5) => {
    ...
    const mapCorrectChar = (brightness: number) => {
      return charSet[Math.floor(p5.map(brightness, thresholds.values[0], thresholds.values[1], 0, charSet.length))];
    };
    ...
    const resizeCanvas = (create = false) => {
      let availableWidth = p5.windowWidth;
      let availableHeight = p5.windowHeight;

      let imageAspectRatio = currentImage.height / currentImage.width;

      let canvasWidth, canvasHeight;

      // Calculate maximum dimensions while maintaining aspect ratio
      if (availableWidth / availableHeight > currentImage.width / currentImage.height) {
    ...
    p5.draw = () => {
      ...
      for (let x = 0; x < xSteps; x++) {
        for (let y = 0; y < ySteps; y++) {
          let xStart = x * gridSquareSize;
          let yStart = y * gridSquareSize;

          const color = currentImage.get(xStart, yStart);
          p5.fill(color);

          const brightness = (p5.red(color) + p5.green(color) + p5.blue(color)) / 3;

          const isInThreshold = brightness >= thresholds.values[0] && brightness <= thresholds.values[1];
          ...
          if (isInThreshold) {
            ...
            const char = debugMode ? brightness.toFixed(0) : mapCorrectChar(brightness);

            p5.textSize(gridSquareSize * 0.9);
            p5.text(char, xStart + gridSquareSize / 2, yStart + gridSquareSize / 2);
          }
        }
      }
